...
function onPos(pos) {
  ...
  if (!best || fix.acc < best.acc) { best = fix; render(fix); }
...
copyBtn.addEventListener('click', async () => {
  ...
  try {
    await navigator.clipboard.writeText(coordsText.value);
...
// Add this function to the end of gps.js (before the closing brace if any):

// Function to send location data back to the opener window
function sendLocationToForm(fix, address) {
  if (window.opener && !window.opener.closed) {
    ...
    window.opener.postMessage(locationData, '*'); // Use specific origin instead of '*' in production
  }
}

// Then modify the finish function in gps.js to call this when a location is obtained:
// In the finish function, after reverseGeocode is called, add:
function finish() {
  clearWatchTimer();
  if (best) {
    render(best);
    ...
    // Send location data back to form
    setTimeout(() => {
      sendLocationToForm(best, lastAddress);
    }, 1000);
  } else progress.textContent = 'No usable fix';
  getBtn.disabled = false; cancelBtn.disabled = true;
}

// Also modify the reverseGeocode function to update lastAddress properly:
// In the reverseGeocode function, after setting lastAddress, add:
// (inside the try block, after lastAddress = addressEl.textContent;)
lastAddress = addressEl.textContent;
if (best && window.opener) {
  sendLocationToForm(best, lastAddress);
}
