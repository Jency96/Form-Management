...
const useBtn = $('useBtn'); // NEW
if (useBtn) useBtn.disabled = true; // start disabled until address is ready



// --- Bridge: post result to parent (index.html modal iframe) ---
function postToParentResult({ lat, lng, acc, address }) {
  const mapsUrl = `https://www.google.com/maps/search/?api=1&query=${lat},${lng}`;
  try {
    // We are inside an <iframe>, send to parent
    if (window.parent) {
...
function clearCurrent() {
  ...
  if (useBtn) useBtn.disabled = true; // disable again until we get a new address
...
// Use in Form -> send to parent and let parent fill the form
if (useBtn) {
...
async function reverseGeocode(lat, lng) {
  try {
    ...
    if (useBtn) useBtn.disabled = false; // NEW: enable once we have an address

  } catch (e) {
    roadEl.textContent = '—'; 
    addressEl.textContent = '—';
    lastAddress = '—'; 
    progress.textContent = 'Address lookup failed';
  }
}
